...
export function LatentMap({
  ...
  return (
    <Card className="lm-card">
      <CardHeader>
        <CardTitle>
          Where the five sit, in the space that grouped them
        </CardTitle>
        <p className="lm-sub">
          Each dot is one article, placed by the meaning of its headline rather
          than by where or when it was published. Distance is similarity:
          articles telling the same story fall together, and that is exactly
          what the engine grouped on.
        </p>
      </CardHeader>
      <CardContent>
        {/* Plot on the left, everything that explains it on the right, once there is
            room for two columns. Centred at 21rem with the key stacked underneath, a
            wide card was half empty and the reader's eye had to travel the full width
            of the page and back between a dot and the name of its group. */}
        <div className="lm-body">
            ...
            <svg
              ...
              aria-label={
                ...
                stories
                  .map(
                    (s) =>
                      `Group ${s.rank}, ${s.actor}, ${s.articles} articles`,
                  )
                  .join(". ") +
          ...
          <div className="lm-side">
            <p className="lm-caption">
              The plane carries <b>{Math.round(facts.retained * 100)}%</b> of
              what separates these five in the full space, and{" "}
              <b>{Math.round(facts.agreement * 100)}%</b> of the dots have their
              nearest neighbour inside their own group. So most of what looks
              close here is close, and the axes themselves mean nothing: only
              the grouping does.
            </p>
            ...
            <ol className="lm-key">
              {stories.map((story, i) => (
                <li key={story.rank}>
                  <span
                    className={`lm-swatch ${RAMP[i % RAMP.length]}`}
                    aria-hidden="true"
                  >
                    {story.rank}
                  </span>
                  ...
                  <span className="lm-n">
                    {story.shown < story.articles ? (
                      <>
                        {story.shown} of{" "}
                        {story.articles.toLocaleString("en-US")} drawn
                      </>
                    ) : (
            ...
            {facts.plotted < facts.articles ? (
              <p className="lm-note">
                ...
                {facts.articles.toLocaleString("en-US")} articles are drawn. A
                long story is sampled evenly through its own order rather than
                truncated, so the shape is the story&rsquo;s and not its first
                hundred sources.
              </p>
            ) : null}
          </div>
